...
try {
  ...
  await axios.put(`http://localhost:3001/update-orders/${updatedOrderInfo.order_id}`, updatedOrderInfo);
...
const handleSoftDeleteOrder = async () => {
  try {
    ...
    await axios.put(`http://localhost:3001/delete-orders/${orderId}`, {
      isdeleted: "true",
    });
    ...
    <ul className="mx-auto mt-12 grid max-w-md grid-cols-1 gap-10 sm:mt-16 lg:mt-10 lg:max-w-3xl lg:grid-cols-3">
      ...
      <li className="flex-start group relative flex lg:flex-col">
        <span
          ...
          aria-hidden="true"
        />
        <div  className={`inline-flex h-10 w-10 shrink-0 text-gray-500  items-center justify-center rounded-xl border border-gray-300 bg-gray-50 transition-all duration-200  ${orderDetails.status === 'OutForDelivery' || orderDetails.status === 'Delivered'? 'bg-my-green text-white' : ''}`}>
          <svg
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
      ...
      <li className="flex-start group relative flex lg:flex-col">
        <div  className={`inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border text-gray-500  border-gray-300 bg-gray-50 transition-all duration-200  ${orderDetails.status === 'Delivered' ? 'bg-my-green  text-white' : ''}`}>
          <svg
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
