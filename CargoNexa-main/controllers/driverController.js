...
const registerDriver = async (req, res) => {
  ...
  const {
    ...
    truck_type, production_year, plate_number, driver_size_type,
  } = req.body;
  ...
  try {
    ...
    const token = jwt.sign(
      { driverId: newDriver.driver_id, driver_email: newDriver.driver_email , role_id:newDriver.role_id},
...
const loginDriver = async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign(
        { driverId: driver.driver_id, driver_email: driver.driver_email , role_id:driver.role_id },
        process.env.SECRET_KEY,
        { expiresIn: '1h' }
      );
